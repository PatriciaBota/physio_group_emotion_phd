...
class VGG(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        ...
        self.conv_layers = self.create_conv_layers(VGG_types['VGG13'])
    ...
    def create_conv_layers(self, architecture):
        ...
        for x in architecture:
            if type(x) == int:
                ...
                layers += [nn.MaxPool1d(kernel_size=(2,2), stride=(2,2))]
                
        return nn.Sequential(*layers)

# test:
def test():
    _in = torch.randn(8, 3, 224)  #[batch, channel, timestamps]
