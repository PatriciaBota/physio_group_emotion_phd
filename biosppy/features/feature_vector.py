...
def get_feat(signal, sig_lab, sampling_rate=1000., windows_len=5, segment=True, save=True):
    ...
    for wind_idx, wind_sig in enumerate(signal):
        if 'EDA' in sig_lab:
            x = np.linspace(0, 1, num=len(wind_sig))
            wind_sig, _ = signals.eda.get_edr_interp(x, wind_sig)
        ...
        if 'EDA' in sig_lab:
            ...
            onsets = signals.eda.get_eda_param(scr)[0]
            _f = nonlinear_geo_features.nonlinear_geo_features(onsets, wind_sig)
            labels_name += [str(sig_lab + i) for i in _f.keys()]
            row_feat += _f.values()

            # Features on EDR Spectrum
            f, spectrum = signals.tools.welch_spectrum(scr, size=len(scr)//2, sampling_rate=sampling_rate)
            # Statistical Features
            _f = statistic_features.signal_stats(spectrum)
            labels_name = [str(sig_lab + 'on_spectrum_' + i) for i in _f.keys()]
            row_feat = _f.values()
            # Temporal Features
            _f = temporal_features.signal_temp(spectrum, sampling_rate)
            labels_name += [str(sig_lab + 'on_spectrum_' + i) for i in _f.keys()]
            ...
            # On SCR Signal
            eda_f = eda_features.eda_features(_eda=wind_sig, edr=scr, sampling_rate=1)
            ...
            if wind_sig is not []:
                ecg_f = ecg_features.ecg_features(wind_sig, sampling_rate=sampling_rate)
...
def FSE(X_train, y_train, features_descrition, classifier, CV=10):
    ...
    for feat_idx, feat_name in enumerate(features_descrition):
        acc_list.append(np.mean(cross_val_score(classifier, X_train[:, feat_idx].reshape(-1,1), y_train, cv=CV, scoring="accuracy")))
    ...
    while 1:
        ...
        for feat_idx, feat_name in enumerate(features_descrition):
            if feat_name not in FS_lab:
                ...
                acc_list.append(np.mean(cross_val_score(classifier, curr_train, y_train, cv=CV, scoring="accuracy")*100))
            else:
                acc_list.append(0)
        curr_acc_idx = np.argmax(acc_list)
