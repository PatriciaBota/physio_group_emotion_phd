...
for MOVIEACQ in _MOVIEACQ:          
    for d in DIM:
        ...
        for lg in ALG:   
            ...
            for eda_tIDX in range(len(_tL)):  # eda dots time
                ...
                if len(idx_ann) > 0:
                   ...
                   #     u_ann += [np.round(sr, 1)]  # users avg annotation
                   # else:
            ...
            for user in range(len(vl)):    
                for j in range(len(emtAnnt[user])):  # annotaions
                    ...
                    if not j and not user and lg== "Scene":
            ...
            plt.plot(u_t, u_ann, ".", label= lg + " Avg. Self-Reports")
            ...
            if len(u_ann) > 1:
                sr = u_ann
                #dstd += [np.sqrt(np.sum((np.array(u_GT)-np.array(sr))**2)/(len(u_GT)-1))]
                dstd += [mean_squared_error(u_GT, sr, squared=False)]
                lk = 2
            ...
            #EDA_fDIC["DTW"] += [np.round(np.nan_to_num(dtw.dtw(stdNorm(u_GT), stdNorm(u_ann))[0]), 2)]
        ...
        plt.savefig("../PLOTS/" + MOVIEACQ + "/60s_AnnvsAvgGT_" + "_" + d + ".pdf", format="pdf", bbox_inches='tight', dpi=300)
        plt.close('all')
