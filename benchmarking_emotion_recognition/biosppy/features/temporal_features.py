...
def signal_temp(signal, FS):
    ...
    if dict['counter']['use'] == 'yes':
        ...
        args += [counter]
        names += ['counter']
    if dict['temp_curve_distance']['use'] == 'yes':
        # counter
        try:
            temp_curve_distance = np.sum(np.linspace(0, signal[-1], len(signal)) - signal)
        except:
            temp_curve_distance = None

        args += [temp_curve_distance]
        names += ['temp_curve_distance']

    if dict['temp_curve_distance_vol']['use'] == 'yes':
        # counter
        try:
            temp_curve_distance_vol = np.trapz(np.linspace(0, signal[-1], len(signal)) - signal)
        except:
            temp_curve_distance_vol = None

        args += [temp_curve_distance_vol]
        names += ['temp_curve_distance_vol']
